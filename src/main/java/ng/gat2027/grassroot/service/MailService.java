...
public class MailService {
    ...
    public MailService(JavaMailSender mailSender, @Value("${spring.mail.host:}") String host, @Value("${gat.mail.from}") String from) {
    ...
    public void sendPasswordReset(String toEmail, String firstName, String resetLink) {
        ...
        String html = "<div style=\"font-family:Segoe UI,Roboto,Arial,sans-serif;max-width:480px;margin:0 auto\">"
            + "<h2 style=\"color:#0b7a3b\">GAT 2027</h2>"
            + "<p>Hi " + escape(firstName) + ",</p>"
            + "<p>We received a request to reset your Grassroot Advocacy for Tinubu account password. Click the button below to choose a new one:</p>"
            + "<p style=\"text-align:center;margin:1.5rem 0\"><a href=\"" + resetLink + "\" style=\"background:#0b7a3b;color:#fff;padding:.75rem 1.5rem;border-radius:8px;text-decoration:none;font-weight:600\">Reset my password</a></p>"
            ...
            + "</div>";
        if (!configured) {
            log.warn("[mail] SMTP not configured — password reset link for {} <{}>: {}", firstName, toEmail, resetLink);
            return;
        }
        try {
